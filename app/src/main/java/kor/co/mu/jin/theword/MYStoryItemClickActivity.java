...
public class MYStoryItemClickActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mystory_item_click);
        CustomActionBar();
        ...
        DatabaseReference dataref = rootref.child("A MYSTORY");
        dataref.orderByChild("title").equalTo(TITLE.getText().toString()).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    MYStoryItemList myStoryItemList = snapshot.getValue(MYStoryItemList.class);
                    CONTENT.setText(myStoryItemList.content);
                    favoriteNum.setText(myStoryItemList.favoriteNumber+"");
                    subwordNum.setText(myStoryItemList.subwordNumber+"");
                }
            }
    ...
    void CustomActionBar() {
        ...
        View CustomActionbar = LayoutInflater.from(this).inflate(R.layout.mystoryitemtitlebar, null);
    ...
    void requestUserInfo() {
        UserManagement.getInstance().me(new MeV2ResponseCallback() {
            ...
            public void onSessionClosed(ErrorResult errorResult) {
                Toast.makeText(MYStoryItemClickActivity.this, "로그인이 되어있지 않습니다.\n댓글 작성 기능이 제한됩니다.", Toast.LENGTH_SHORT).show();
        ...
        if(subword.length() <= 0 ){
            ...
            new AlertDialog.Builder(this).setMessage("해당 내용으로 댓글을 등록하시겠습니까 ??").setNegativeButton("취소",null).setPositiveButton("확인", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    final DatabaseReference dataref = databaseReference.child("A MYSTORY");
                    dataref.orderByChild("title").equalTo(TITLE.getText().toString()).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            for(DataSnapshot snapshot : dataSnapshot.getChildren()){
